...
def import_module(args: List, node):
    ...
    if not isinstance(module_name, str):
        evaluator.error("1st param must be of type str", node)
    ...
    if module_exists(mplispstd):
        ...
        evaluator.error("package {} not found".format(module_name), node)
...
def python_getattr(args: List, node):
    ...
    if not isinstance(params[1], str):
        evaluator.error("2st param must be of type str, {} given".format(type(params[1])), node)
